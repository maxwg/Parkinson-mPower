function [measures_vector, measures_names, F0] = voice_analysis_f(data, fs, f0_alg)
%
...
f0max = 400; % max F0 possible --- Adjust depending on application!
flag = 1;
...
F0mean_healthy_control = 150; % mean(F0); % strictly speaking, adjust this for healthy controls males = 120 and females = 190
...
f0max=400; %Hz -- max value, possibly adjust for other applications
f0min=50; %Hz
tstep=0.01*fs;
x=0.08*fs;
...
IMF_dec = emd(data, 'STOP',[0.05,0.5,0.05],'MAXITERATIONS',30);
IMF_dec=IMF_dec';
