"""
This file is essentially the same as path_processors
with some slight modifications for the DREAM challenge.
...
def butter_highpass_filter(data, lowcut, freq, order=5):
    ...
    """
    b, a = _butter_highpass(lowcut, freq, order=order)
    y = filtfilt(b, a, data)
    return y
...
def loadMotionIntoArray(path, highPass=False, crop=(400, 2000)):
    """
...
def getAllFeaturesRest(path, highPass=True):
    ...
    lyap_x_Eck=nolds.lyap_e(x,tau= xTau, emb_dim=6, matrix_dim=6)
    lyap_x_Ros=nolds.lyap_r(x, tau=xTau, emb_dim=6)
    lyap_y_Eck=nolds.lyap_e(y,tau= yTau, emb_dim=6, matrix_dim=6)
    lyap_y_Ros=nolds.lyap_r(y, tau=yTau, emb_dim=6)
...
def featuresRestNames():
    ...
    dynamic = ["xTau", "yTau", "higuchi_x", "higuchi_y", "hurst_x", "hurst_y", "pfd_x", "pfd_y", "dfa_x", "dfa_y",
               "rpde_x", "rpde_y",
               "LLE_Rosen_x", "Lyap_Eck_x_1", "Lyap_Eck_x_2", "Lyap_Eck_x_3", "Lyap_Eck_x_4", "Lyap_Eck_x_5", "Lyap_Eck_x_6",
               "LLE_Rosen_y", "Lyap_Eck_y_1", "Lyap_Eck_y_2", "Lyap_Eck_y_3", "Lyap_Eck_y_4", "Lyap_Eck_y_5", "Lyap_Eck_y_6"]
    features['dynamic'] = dynamic
    ...
    features['hjorth'] = ["x_activity", "x_complexity", "x_morbidity", "y_activity", "y_complexity", "y_morbidity"]
...
def getAllFeaturesWalking(path):
    ...
    lyap_y_Eck=nolds.lyap_e(y,tau= yTau, emb_dim=6, matrix_dim=6)
    lyap_y_Ros=nolds.lyap_r(y, tau=yTau, emb_dim=6)
    lyap_z_Eck=nolds.lyap_e(z,tau= zTau, emb_dim=6, matrix_dim=6)
    lyap_z_Ros=nolds.lyap_r(z, tau=zTau, emb_dim=6)
    dynamic.extend(lyap_x_Eck)
    dynamic.append(lyap_x_Ros)
    dynamic.extend(lyap_y_Eck)
    dynamic.append(lyap_y_Ros)
