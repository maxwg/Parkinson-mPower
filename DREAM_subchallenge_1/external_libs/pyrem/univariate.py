...
def _make_cm(X,M,R):
    ...
    inrange_last = np.concatenate((inrange_last, [False]))
    Cm[inrange_last] += 1
...
def samp_entropy(a, m, r, tau=1, relative_r=True):
    ...
    for i,template in enumerate(embs_mini[:-1]):
        ...
        in_range_sum = int(np.sum(range_b_view))
        sum_cm  += in_range_sum
        ...
        range_b_view = np.concatenate((range_b_view, [False]*i)).astype(np.bool)
...
def spectral_entropy(a, sampling_freq, bands=None):
    ...
    if bands is None:
        ...
        power_per_band = np.array([np.sum(psd[np.bitwise_and(freqs >= low, freqs<up)])
